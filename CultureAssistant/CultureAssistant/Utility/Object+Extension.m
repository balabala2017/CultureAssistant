...
+ (NSString *)extractBirthday:(NSString *)cardNum
{
    NSString *string = @"";
    
    if (cardNum.length == 15) {
        NSString* year = [NSString stringWithFormat:@"%d",[cardNum substringWithRange:NSMakeRange(6,2)].intValue +1900];
        NSString* month = [cardNum substringWithRange:NSMakeRange(8,2)];
        NSString* day = [cardNum substringWithRange:NSMakeRange(10,2)];
        string = [NSString stringWithFormat:@"%@-%@-%@",year,month,day];
    }
    if (cardNum.length == 18) {
        NSString* year = [cardNum substringWithRange:NSMakeRange(6,4)];
        NSString* month = [cardNum substringWithRange:NSMakeRange(10,2)];
        NSString* day = [cardNum substringWithRange:NSMakeRange(12,2)];
        string = [NSString stringWithFormat:@"%@-%@-%@",year,month,day];
    }
    return string;
}
